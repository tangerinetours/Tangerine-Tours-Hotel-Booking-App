...
export default function PaymentResultContent() {
  ...
  useEffect(() => {
    ...
    const bookingDetails = JSON.parse(sessionStorage.getItem("mpgs_booking") || "{}");
      ...
      .then((data) => {
        if (data.success) {
          fetch("/api/send-booking-email", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ bookingDetails, orderId, amount }),
          }).catch((e) => console.error("Email send failed:", e));

          setStatus("success");
          setMessage("Your payment was successful! Your booking is confirmed. A confirmation email has been sent.");
        } else {
          setStatus("failed");
